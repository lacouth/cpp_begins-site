...
int main() {
    ...
    VERIFICA(ultima.has_value());
    VERIFICA_IGUAL(ultima.value(), std::string("perfil"));
    VERIFICA_IGUAL(h.total_visitas(), 2);

    HistoricoNavegacao vazio;
    VERIFICA(!vazio.ultima_visitada().has_value());
    VERIFICA_IGUAL(vazio.total_visitas(), 0);
